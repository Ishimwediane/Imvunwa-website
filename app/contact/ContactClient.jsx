...
/* ── Contact page body (client) ──────────────────────────────── */
export default function ContactClient({ info }) {
  const digits = (info.phone || "").replace(/\D/g, "");
  const CONTACT_INFO = [
    { tag: "a", href: `tel:${(info.phone || "").replace(/\s+/g, "")}`, icon: "📞", text: info.phone },
    { tag: "a", href: `mailto:${info.email}`, icon: "✉️", text: info.email },
    { tag: "span", icon: "📍", text: info.address },
    { tag: "span", icon: "🕐", text: info.hours },
  ];
